...
async function pickUntilSortIsDeterministic(
    window: typeof Window, selectedProperties: string[], quickPickItems: QuickPickItem[], arrayToSort: GenericObject[], sortOrder: SortOrder):
  ...
  if (selectedItem != null) {
    ...
    const sortIsDeterminstic = sortObjectArray(arrayToSort, selectedProperties, sortOrder);
    ...
    if (sortIsDeterminstic || selectedProperties.length === quickPickItems.length) {
      ...
      return await pickUntilSortIsDeterministic(window, selectedProperties, quickPickItems, arrayToSort, sortOrder);
    }
  }
}
export async function sortObjects(window: typeof Window, array: GenericObject[], sortOrder: SortOrder): Promise<GenericObject[] | undefined> {
  ...
  if (quickPickItems.length === 0) {
    ...
    return await pickUntilSortIsDeterministic(window, [], quickPickItems, array.slice(0), sortOrder);
  }
}
